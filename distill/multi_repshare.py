...
class MultiOnlineRepDistiller(MultiOnlineRepDistiller):
  """
  ...
  def distill_loop(self):
    ...
    with self.summary_writer.as_default():
      ...
      for _ in tf.range(num_epochs):
        one_epoch_iterator = (next(self.train_batch_iterator) for _ in range(self.student_task.n_train_batches))
        one_epoch_iterator = (next(self.train_batch_iterator) for _ in range(self.student_task.n_train_batches))
